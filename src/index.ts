...
import type { Wallet } from "../lib-bindings/lib";

const getWallet = (() => {
  let cls: typeof Wallet | null = null;
  return async () => {
    if (!cls) {
      const { Wallet } = await import("../lib-bindings/lib");
      cls = Wallet;
    }
    return cls;
  };
...
ipcMain.handle("wallet", async () => {
  return (await getWallet()).derive_new().address();
});
